...
--
-- ATENÇÃO: as seções marcadas com [REQUER MIGRATION] só funcionam
-- após executar as migrations 005 e 006.
-- ============================================================


-- ── 1. RESUMO GERAL — contagem em todas as tabelas ───────────────────────────
...
UNION ALL
SELECT 'aditivos (estruturada)',       COUNT(*) FROM aditivos
ORDER BY tabela;


-- ── 2. raw_contratos — cobertura dos campos cruciais ────────────────────────
...
SELECT
    ...
    -- Distribuição de situação inferida
...
SELECT
    ...
    COUNT(*) FILTER (WHERE data_publicacao IS NULL
                        AND data_publicacao_edital IS NULL)        AS sem_data,
...
-- ── 4. raw_obras_saude (SISMOB) — cobertura ─────────────────────────────────
...
-- ── 5. raw_obras_georef (EGIM) — cobertura ──────────────────────────────────
-- Colunas data_inicio, setor_administrativo, objectid requerem migration 006
...
-- ── 8. raw_obras_paralisadas — cobertura ────────────────────────────────────
...
-- ── 9. obras (estruturada) — cobertura e médias ──────────────────────────────
...
-- ── 10. contratos (estruturada) — situação por fonte ────────────────────────
...
-- ── 11. obras — distribuição de situação ────────────────────────────────────
...
-- ── 12. obras SEM percentual (deve estar vazio após fallbacks) ───────────────
...
-- ── 13. contratos SEM situação (deve estar vazio após fallbacks) ─────────────
