...
Autenticação:
    Header: chave-api: <TRANSPARENCIA_API_KEY>
...
def _get(endpoint: str, params: dict[str, Any]) -> JSONValue:
    ...
    for attempt in range(1, RETRY_ATTEMPTS + 1):
        try:
            ...
            log.warning(f"Timeout na tentativa {attempt}/{RETRY_ATTEMPTS}: {url}")
        except requests.exceptions.ConnectionError:
            log.warning(f"Erro de conexão na tentativa {attempt}/{RETRY_ATTEMPTS}: {url}")
...
def fetch_contratos() -> list[Record]:
    ...
    params = {
        "municipioCodigoIbge": MUNICIPIO_CODE,
    }
